...
- **鼠标控制**：移动鼠标到指定坐标、执行左键/右键点击，操作后自动返回截图
- **键盘操作**：输入文本、按下单个按键或组合键，操作后自动返回截图
- **屏幕操作**：获取屏幕截图（AI 可直接看到图片内容）、获取屏幕尺寸和鼠标位置信息
- **WebSocket 服务**：等待本地控制端主动连接
- **Agent 集成**：支持通过 AstrBot 的 Agent 系统调用控制工具，截图以 ImageContent 格式返回，AI 可直接解析图片内容
...
- v1.0.2：截图工具返回 ImageContent 格式，AI 可直接看到图片内容；清理调试代码
